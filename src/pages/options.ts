...
import { getSheetData, disconnect } from '../services/oauth';
import { renderElement, renderTree } from '../lib/renders';
import { activateTooltips, renderPreviewTooltips, tooltip } from '../components/tooltip';
import { 
  getCurrentTab, 
  applyDefaultMetrics,
  applyDefaultStyles, 
...
const CUSTOM_DATA_TO_SAVE = {
  [CUSTOM_DATA_TYPES.BATTER as string]: {},
  [CUSTOM_DATA_TYPES.PITCHER as string]: {},
  [CUSTOM_DATA_TYPES.LIMIT as string]: {},
}
...
const setFormState = async () => {
  ...
  (CONNECT_SHEET_ID_INPUT as HTMLInputElement).value = sheetId;
  (CONNECT_BATTER_RANGE_INPUT as HTMLInputElement).value = batterRange;
  (CONNECT_PITCHER_RANGE_INPUT as HTMLInputElement).value = pitcherRange;
  (CONNECT_LIMIT_RANGE_INPUT as HTMLInputElement).value = limitRange;
  validateInputFields();
};

const getSavedData = async () => {
  const savedBatterData = await getStorageItem(STORAGE_KEYS.customBatterData);
  if (Object.keys(savedBatterData).length > 0) {
    renderSavedData(savedBatterData, BATTERS_SAVED);
  }
  const savedPitcherData = await getStorageItem(STORAGE_KEYS.customPitcherData);
  if (Object.keys(savedPitcherData).length > 0) {
    renderSavedData(savedPitcherData, PITCHERS_SAVED);
  }
  const savedLimitData = await getStorageItem(STORAGE_KEYS.customLimitData);
...
const renderDataTable = (
  ...
  let rowCount = 0;
  customDataValues.forEach((entry, i) => {
...
//      const hideErrorsTooltip = hideErrors ? translation.options.showErrorsTooltip : translation.options.hideErrorsTooltip;
...
/*
      const tt = tooltip({ 
        type: 'info',
        tooltip: translation.options.removeDuplicatesTooltip,
      }) as ElementDefinition;
*/
...
//        children: [removeDuplicatesAction, tt],
        style: { display: "inline-block" },
      }
...
const renderDraftData = (
  ...
  const dataTable = renderDataTable(customDataObject, customTableRecord, hideErrors);
