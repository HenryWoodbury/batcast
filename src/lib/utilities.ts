...
import { validateDataObject } from './validations';
...
export const getStoredTab = async () => {
  const tabId = await getStorageItem(STORAGE_KEYS.tabId);
...
export const applyDefaultStyles = async (tab: chrome.tabs.Tab, action: storageInsertActions) => {
...
export const applyDefaultMetrics = async (tab: chrome.tabs.Tab, action: storageInsertActions) => {
  if (action === storageInsertActions.SET) {
...
export const createDataObject = (
  ...
  const hasOnlyValidKeys = arrayKeys.every(val => dataKeys.includes(val.replace(/\s+/g, '')));
  if (!hasOnlyValidKeys) {
...
export const getValidData = (
  ...
  for (const key in dataObject) {
    ...
    customDataObject.columns.forEach((column, i) => {
...
export const getPitcherImpact = (
  ...
  if (pitcher.OttoneuID === '21015') {
    console.log(pitcher.OttoneuID);
    console.log(pitcher.FIP);
    console.log(vsRHBc);
    console.log(vsLHBc);
    console.log('---')
    console.log(pitcher.Throws);
    console.log(pitcher.vsLHB);
    console.log(pitcher.vsRHB);
  }
  return {
